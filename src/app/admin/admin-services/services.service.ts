...
private apiUrl='https://localhost:44353/api';
  ...
  suadanhmuc(id:number, formData: FormData ){
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/DanhMucSanPham/SuaDanhMuc/${id}`, formData , {responseType:'text',headers});
  }
  xoadanhmuc(id:number): Observable<any>{
    const headers = this.getAuthHeaders();
    return this.http.delete(`${this.apiUrl}/DanhMucSanPham/XoaDanhMuc/${id}`, {responseType:'text',headers});
  }
  ...
  getchucvu(): Observable<ChucVu[]>{
   ...
   getbychucvuid(id:number){
    const headers = this.getAuthHeaders();
    return this.http.get<ChucVu>(`${this.apiUrl}/ChucVu/getbyidchucvu/${id}`,{headers});
   };

   suachucvu( id: number, data: ChucVu){
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/ChucVu/suachucvu/${id}`, data, {responseType:'text',headers});
   };
  ...
  xoasanpham(id:number): Observable<any> {
    const headers = this.getAuthHeaders();
   return this.http.delete(`${this.apiUrl}/SanPhamChiTiet/xoasanpham/${id}`, {responseType:'text', headers})
  };
  //tesst
  ...
  xoaanh(id:number){
   ...
   detailOrder(id:any):Observable<hoadon>{
    const headers = this.getAuthHeaders();
    return this.http.get<hoadon>(`${this.apiUrl}/HoaDon/detailOrder/${id}`,{headers});
   }
   listOrder(id:any):Observable<hoadonchitiet[]>{
    const headers = this.getAuthHeaders();
    return this.http.get<hoadonchitiet[]>(`${this.apiUrl}/HoaDon/ListOrder/${id}`,{headers});
   }
   trangthaidonhang(formData: FormData){
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/HoaDon/CapnhatTT`, formData,{headers});
   }
   xacnhadonhang():Observable<hoadon[]>{
    const headers = this.getAuthHeaders();
    return this.http.get<hoadon[]>(`${this.apiUrl}/HoaDon/choxacnhan`,{headers});
   }

   xacnhanall(){
    const headers = this.getAuthHeaders();
    return this.http.post(`${this.apiUrl}/HoaDon/xacnhanall`,{responseType:'text'},{headers});
   }
